/* Original code[1] Copyright (c) 2015 Simon Wittber[2]
   Licensed under the MIT License[3]

   This comment generated by code-cite[4].

   [1]: https://github.com/simonwittber/uGIF
   [2]: https://github.com/simonwittber
...
namespace uGIF
{
  public class GIFEncoder
  {
    ...
    public void AddFrame (Image im)
    {
      ...
      pixels = im.pixels;
      if ((firstFrame && nq == null) || !useGlobalColorTable)
        BuildPalette(im);
    ...
    public void BuildPalette(IEnumerable<Image> samples) {
      var pixels = new List<Color32>();
      foreach (var sample in samples)
        pixels.AddRange(sample.pixels);
      nq = new NeuQuant (pixels.ToArray(), pixels.Count, quality);
      colorTab = nq.Process (); // create reduced palette
    }

    public void BuildPalette(Image image) {
      var pixels = image.pixels;
      nq = new NeuQuant (pixels, pixels.Length, quality);
      colorTab = nq.Process (); // create reduced palette
    }
    ...
    void RemapPixels ()
    {
      ...
      // if ((firstFrame || !useGlobalColorTable) && nq == null && colorTab == null) {
      //  // initialize quantizer
      //   Debug.Log("new NeuQuant");
    ...
    void WriteImageDesc ()
    {
      ...
      // Need to say what size the local palette is.
      if (firstFrame || useGlobalColorTable)
        ms.WriteByte (0);
      else if (! useGlobalColorTable) {
        byte b = (byte) ((1 << 7) // local color table flag
                         | palSize); // size of local color table
        ms.WriteByte(b);
      }
